...
- **Global scope**: `C:/project/temp/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/project/temp/.claude/notes/`)
...
2. Glob `C:/project/temp/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/project/temp/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/project/temp/.claude/notes/` — works in any directory
